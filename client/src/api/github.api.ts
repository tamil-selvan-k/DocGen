import client from './client';
import { ENDPOINTS, API_BASE_URL } from '@/constants/api.constants';
import type { ApiResponse } from '@/types/api.types';
import type { Organization, GitHubInstallation } from '@/types/github.types';

export const githubApi = {
  /** Returns full connect URL — browser navigates to this to start OAuth */
  getConnectUrl: (): string => `${API_BASE_URL}${ENDPOINTS.GITHUB_CONNECT}`,

  /** Returns full App installation URL — browser navigates to this to install/configure repositories */
  getInstallUrl: (): string => `${API_BASE_URL}${ENDPOINTS.GITHUB_INSTALL}`,

  listOrganizations: () =>
    client.get<ApiResponse<Organization[]>>(ENDPOINTS.ORGANIZATIONS),

  listInstallations: () =>
    client.get<ApiResponse<GitHubInstallation[]>>(ENDPOINTS.INSTALLATIONS),
};
