...
async function processDocJob(job: Job<DocJobData>): Promise<void> {
  ...
  try {
    ...
    await GitHubClient.createOrUpdateFile(
      ...
      docFilePath,
      generatedDocs,
      `docs: auto-update ${docFilePath} for commit ${commitSha.slice(0, 7)} [DocGen AI]`,
      branchName,
    ...
    const prBody = [
      '## DocGen AI — Automated Documentation Update',
      '',
      `This PR was automatically generated by **DocGen AI** in response to commit [\`${commitSha.slice(0, 7)}\`](https://github.com/${owner}/${repo}/commit/${commitSha}).`,
      '',
      '### Changed Files',
      ...
      `- \`${docFilePath}\``,
      '',
      '> ⚠️ Please review before merging. DocGen AI only includes facts derived directly from the code diff.',
    ].join('\n');
